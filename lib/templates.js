var nTemplates = function (story, world, storyGen) {
  ...
  story.title = function (god) {
    ...
    if (
      ...
      god.villain.health === world.healthLevel.alive &&
      god.villain.introduced
    ) {
      subject = god.villain;
    ...
    var templates = [
      ...
      (god.coinflip() ? "The Adventures of " : "") +
        "{{SN}} of <%= hero.home.nation %>",
      (god.coinflip() ? "A certain " : "") +
        "{{SN}} and what {{PRON}} did" +
        (god.coinflip() ? " in <%= hero.home.nation %>" : ""),
    ];

    if (god.villain.introduced) {
      templates.push(
        (villForm ? "The {{VF}} " : "") +
          "{{VN}} and what happened to <%= pronounobject(villain) %>",
        "The {{TL}} of {{HN}} and " +
          (villForm ? "the {{VF}} " : "") +
          "{{VN}}",
        "{{HN}} and {{VN}}",
    ...
    if (god.cache.villains) {
      ...
      var vills =
        "{{HN}} and the {{DFT}} of " +
        god.list(god.cache.villains, function () {
          return god.coinflip() ? "name" : "nickname";
        });
      vills = vills.replace(/{{DFT}}/gm, god.pick(defeated));
      // console.log(vills);
    ...
    return t
      .replace(/{{TL}}/gm, tale)
      .replace(/{{PRON}}/gm, pron)
      .replace(/{{POSS}}/gm, poss)
      .replace(/{{SN}}/gm, sn)
      .replace(/{{VF}}/gm, villForm)
      .replace(/{{MI}}/gm, mi);
  };
  ...
  story.introduceHero = function (god, terse) {
    var near = god.select(
      ...
      "within a days walk of",
    );
    var nationType = god.select(
      "country",
      "province",
      "kingdom",
      "nation",
    ...
    var templates = [
      ...
      "a certain {{MW}} was very <%= pick(hero.description) %>. " +
        (god.coinflip()
          ? "<%= capitalize(possessive(hero)) %> name {{was}} {{HN}}."
          : "{{HN}} <%= possessive(hero) %> name {{was}}."),
    ...
    if (terse) {
      t.push(
        god
          .pick(templates)
          .replace("{{NT}}", nationType)
          .replace("{{NEAR}}", near),
      );
    } else {
      t.push(
        (god.coinflip() ? story.intro(god) + " " : "") +
          god
            .pick(templates)
            .replace("{{NT}}", nationType)
            .replace("{{NEAR}}", near),
      ...
      t.push(
        blankLine,
        '<%= hero.name %> {{lived}} with <%= list(hero.family, "nickname") %>.',
      );
      if (god.hero.acquaintances.length > 0) {
        if (god.hero.acquaintances.length > 1) {
          t.push(
            blankLine,
            '<%= list(hero.acquaintances, "nickname") %> {{were}} <%= select("friends of", "known to") %> <%= hero.name %>.',
          );
        } else {
          t.push(
            blankLine,
            '<%= list(hero.acquaintances, "nickname") %> {{was}} <%= select("a friend of", "known to") %> <%= hero.name %>.',
          );
        }
      }
  ...
  story.createVictim = function (god) {
    var victim = god.getCharacter(
      god.pick(god.hero.family.concat(god.hero.acquaintances)),
    );
  ...
  story.introduceVillain = function (god) {
    ...
    return god
      ...
      .replace(/{{B}}/gm, B)
      .replace(/{{A}}/gm, A)
      .replace(/{{TIME}}/gm, time)
      .replace(/{{PP}}/gm, person);
  };
  ...
  story.deathResponse = function (god, person) {
    ...
    var templates = [
      "It happens to everyone eventually. It {{happened}} to " +
        god.pronounobject(person) +
        " sooner.",
      "These things happen. " +
        (god.coinflip() ? "Unfortunately." : "It is unfortunate."),
    ...
    if (god.coinflip()) {
      t.push("There {{was}} much wailing in " + god.hero.home.vicinity + ".");
    }
  ...
  story.subtale = function (narrator, god) {
    ...
    var presets = god.coinflip()
      ? storyGen.presets.shortWaterStory
      : world.util.randomProperty(storyGen.presets);
    ...
    return sg.generate(setts, god.theme);
  };
  ...
  story.createLack = function (god) {
    ...
    var lacks = [
      "{{needs}} a bride, a friend, or just somebody to talk to",
      "{{needs}} a helper or magical agent",
      [
        "{{needs}} a wondrous object or two",
  ...
  story.punish = function (god, person) {
    ...
    if (god.hero.possessions && god.hero.possessions.length > 0) {
      mi = god.hero.possessions[god.hero.possessions.length - 1];
      var mit = (
        "Seeing that {{VN}} was perfectly enfeebled, {{HN}} snatched up {{HPRNO}} " +
    ...
    var dispersal = [
      (god.coinflip() ? "Thanks to {{HN}}, " : "") +
        "{{PN}} was completely burnt to cinders.",
      "{{AS}}that {{was}} that.",
    ...
    if (god.coinflip()) {
      var tmpl2 = [
        'God <%= (coinflip() ? "evidently " : "")%>{{did}} it to <%= select("punish", "remonstrate", "educate", "rebuke") %> {{PN}} for {{POSS}}' +
          '<%= (coinflip() ? " great" : "")%> <%= nlp.adjective("' +
          descr +
          '").conjugate().noun %>.',
        "{{BA}}{{PN}} sits to this day in the pit - in Tartarus.",
        "{{BA}}{{PN}} " +
          (god.coinflip()
            ? '<%= select("{{disappear}}", "{{vanish}}") %>, and '
            : "") +
    ...
    return t
      .join(" ")
      .replace(/{{PN}}/gm, function () {
        return eitherName(person);
      })
      .replace(/{{AS}}/gm, as)
      .replace(/{{BA}}/gm, ba)
      .replace(/{{POSS}}/gm, poss)
      .replace(/{{PROO}}/gm, proo)
      .replace(/{{HPN}}/gm, hpron)
      .replace(/{{HPRNO}}/gm, hprono)
      .replace(/{{VPRO}}/gm, vpro);
  };
  ...
  story.intro = function (god) {
    ...
    var intros = [
      ...
      "Once upon a time,",
      (god.coinflip()
        ? 'We <%= coinflip() ? "like to " : "" %><%= coinflip() ? "say" : "think" %> ' +
          ...
          "stories tell that "
        : "") +
        "before our grandfathers had learnt anything, and before their grandfathers were born,".replace(
          /fathers/gm,
          god.coinflip() ? "fathers" : "mothers",
        ),
    ];
  ...
  story["func1"].exec = function (god, subFunc) {
    ...
    if (!god.hero.introduced) {
      t.push(story.introduceHero(god, tersely));
    }

    if (!god.cache.victim) {
      story.createVictim(god);
    }

    var templates = [
      "{{VICN}} went missing.",
      "{{VICN}} unexpectedly {{died}}, leaving <%= hero.name %> devastated." +
        (god.coinflip()
          ? " " + story.deathResponse(god, god.cache.victim)
          : ""),
      "Sooner or later, {{VICN}} {{died}}." +
        (god.coinflip()
          ? " " + story.deathResponse(god, god.cache.victim)
          : ""),
    ];
  ...
  story["func2"].exec = function (god) {
    ...
    if (!god.hero.introduced) {
      text.push(story.introduceHero(god, tersely));
    }
    ...
    switch (hero.interdiction.type) {
      case world.interdictionType.movement:
        ...
        text.push(
          "<%= advisor.name %> warned <%= hero.name %> to avoid <%= randomProperty(hero.interdiction.place) %>.",
        );
        break;
      ...
      case world.interdictionType.action:
        // interdiction.action = 'take the Lord\'s name in vain AGAIN';
        text.push(
          "<%= advisor.name %> told <%= hero.name %> to not <%= hero.interdiction.action %>.",
        );
        break;
      ...
      case world.interdictionType.speak:
        ...
        // that way, we can "travel" to target....
        text.push(
          "<%= hero.interdiction.advisor.name %> warns <%= hero.name %> to not <%= hero.interdiction.action %>.",
        );
        break;
    }
    ...
    text.push(
      world.blankLine,
      hero.interdiction.advisor.name + " introduced {{MH}} to " + hero.name,
    );
    ...
    text.push(
      world.blankLine,
      '"And now," {{concluded}} {{MH}}, "my tale is done."',
    );
  ...
  story["func3"].exec = function (god) {
    ...
    if (!interdiction) {
      ...
      console.log(
        "INTERDICTION IS NOT DEFINED IN FUNC3; was func2 skipped??? YOU NEED TO EXTRACT THAT TO A METHOD",
      );
      return " ";
    }
    ...
    switch (interdiction.type) {
      case world.interdictionType.movement:
        text.push(
          "Despite the warning, <%= hero.name %> went to <%= randomProperty(hero.interdiction.place) %>.",
        );
        text.push(
          world.blankLine,
          "<%= villain.name %>, a rather <%= list(villain.description) %> person, appeared.",
        );
      ...
      case world.interdictionType.action:
        text.push(
          "Shockingly, <%= hero.name %> proceeded to <%= hero.interdiction.action %>.",
        );
        text.push(
          world.blankLine,
          "<%= villain.name %>, a rather <%= list(villain.description) %> person, appeared in front of <%= hero.name %>.",
        );
        break;

      case world.interdictionType.speak: {
        var t =
          "As soon as <%= hero.interdiction.advisor.name %> {{was}} gone, <%= hero.name %> " +
  ...
  story["func5"].exec = function (god) {
    ...
    t.push(god.pick(tmpls).replace(/{{VN}}/gm, vn));
  ...
  story["func7"].exec = function (god) {
    var text = [];
    if (!god.villain.introduced) {
      text.push(story.introduceVillain(god));
    }
    if (!god.hero.introduced) {
      text.push(story.introduceHero(god, tersely));
    }
  ...
  story["func8"].exec = function (god, subFunc, skipIntros) {
    ...
    if (!god.hero.introduced && !skipIntros) {
      template.push(story.introduceHero(god, tersely));
    }
    ...
    switch (subFunc) {
      case "kidnapping of person": {
        // o, god!
        var kp = god.getCharacter(
          god.pick(god.select(god.hero.family, god.hero.acquaintances)),
        );
        ...
        template.push(
          "{{VN}} kidnapped {{NAME}}{{FO}}."
            .replace(/{{NAME}}/gm, name)
            .replace(/{{FO}}/gm, friendof),
        );
        break;
      }
      ...
      case "seizure of magical agent or helper":
        template.push(
          '{{VN}} <%= select("forcibly seized", "kidnapped", "{{makes}} off with") %> <%= magicalhelper.name %>.',
        );
        break;
      ...
      case "pillaging or ruining of crops":
        template.push(
          "The harvest {{was}} destroyed by {{VN}}. All in <%= hero.home.nation %> {{begins}} to feel the pangs of hunger.",
        );
        break;
      ...
      case "bride is forgotten":
        ...
        template.push(
          "{{HN}}'s bride {{was}} forgotten after {{VN}} cast a spell.",
        );
        break;

      case "demand for delivery or enticement, abduction":
        template.push(
          "{{VN}} made a demand for delivery or enticement, abduction. Something like that.",
        );
        break;

      case "expulsion":
        template.push(
          "{{HN}} {{was}} driven from <%= possessive(hero.gender) %> <%= hero.home.residence %>.",
        );
        break;
      ...
      case "casting into body of water": {
        ...
        var wet = [
          ...
          [
            "{{HN}} was such an awful drunkard as passes all description. " +
              ...
              "tumbled into the water - and there was an end of <%= hero.object%>.",
            false,
            true,
          ],
        ];
        ...
        if (survives && (god.hero.canswim || flip)) {
          ...
          god.hero.health = world.healthLevel.dead;
          var drowns = [
            god.select("Too bad ", "Unfortunately, ", "Regretably, ", "", "") +
              "{{HN}} had " +
              (god.coinflip()
                ? "never learned to swim."
                : "a considerable attachment to breathing."),
          ];
          if (!skipDeath) {
      ...
      case "issues order to kill [requires proof]":
        template.push(
          "{{VN}} {{issues}} an order to kill. It {{requires}} proof. THIS {{WAS}} CRUEL.",
        );
        break;
      ...
      case "commits murder": {
        ...
        var sudden = god.select(
          ...
          "without explanation",
          "since <%= pronoun(villain) %> {{was}} unstoppable",
        );
      ...
      case "threat of forced matrimony":
        template.push(
          "{{VN}} {{threatened}} to marry <%= getCharacter(pick(hero.family)).name %>.",
        );
        break;

      case "threat of forced matrimony between relatives":
        template.push(
          '{{VN}} {{<%= select("insinuated", "suggested", "mused") %>}} that <%= list(hero.family) %> could be forced into a marriage of convenience.',
        );
        break;
      ...
      case "threat of cannibalism among relatives":
        template.push(
          "Thanks to the ravages {{VN}}'s predations had left on the land, there {{was}} the threat of cannibalism among the relatives of {{HN}}'s family. <%= list(hero.family) %> eyed each other hungrily.",
        );
        break;
      ...
      case "tormenting at night (visitation, vampirism)": {
        ...
        var tormenter =
          god.villain.family.length > 0
            ? god.pick(god.villain.family)
            : god.villain;
        template.push(
          "{{HN}} {{was}} tormented at night by " + tormenter.name + ".",
        );
        break;
      }
    ...
    if (!skipVillain && !skipIntros) {
      if (!god.villain.introduced) {
        template.unshift(story.introduceVillain(god));
      }
    }
  ...
  story["func9"].exec = function (god) {
    ...
    var sent =
      "{{HN}} {{discovered}} that {{PN}}{{FO}} " + god.cache.lack.lack + ".";

    return sent.replace(/{{PN}}/gm, name).replace(/{{FO}}/gm, friendof);
  };
  ...
  story["func11"].templates.push(
    "<%= hero.name %> left <%= hero.location %> to <%= task %>.",
  );
  ...
  story["func14"].exec = function (god, item) {
    ...
    var met =
      '<%= select("met", "encountered", "came across", "found", "was found by", "bumped into") %>';
    ...
    t.push(
      '"What does it look like?" replied {{AN}}. "It\'s a special, magical {{IT}}. Perhaps you can use it in your struggle with {{VN}}."',
    );
    if (god.coinflip()) {
      t.push(
        '"Thanks!" said a <%= select("grateful", "thankful") %> {{HN}}' +
    ...
    if (god.coinflip(0.3)) {
      ...
      t.push(
        world.blankLinke,
        '"And now," {{concluded}} {{AN}}, "my tale is done."',
      );
  ...
  story["func16"].exec = function (god) {
    ...
    return t
      ...
      .replace(/{{VF}}/gm, villForm)
      .replace(/{{VO}}/gm, vo)
      .replace(/{{VPOSS}}/gm, vposs)
      .replace(/{{VPRON}}/gm, vpron)
      .replace(/{{HO}}/gm, ho)
      .replace(/{{HPRON}}/gm, hpron)
      .replace(/{{HPOSS}}/gm, hposs)
      .replace(/{{HRP}}/gm, hrp);
  };
  ...
  story["func19"].exec = function (god, subFunc) {
    ...
    var tmpls = [
      "The problems experienced by " +
        god.getCharacter(god.cache.lack.person).name +
        ", who {{LT}},  {{were}} resolved by {{HN}}.",
    ];
  ...
  story["func23"].templates.push(
    "<%= hero.name %> arrived in <%= hero.home.vicinity %> but {{was}} unrecognized.",
  );

  // Claim: False hero makes unfounded claims
  story["func24"].templates.push(
    "<%= falsehero.name %> made unfounded claims.",
  );
  ...
  story["func30"].exec = function (god, subFunc) {
    ...
    var person =
      god.villain.health === world.healthLevel.living
        ? god.villain
        : god.falsehero;
  ...
  story["func31"].exec = function (god, subFunc) {
    ...
    var templates = [
      ...
      // this is the... what tense? past would work.
      // if ALL verb ar infinitive and appear as {{verb}}, then we do a global pull, conjugate, replace prior to template parsing. or after. whatever.
      "<%= select(marriage, ascension) %>, <%= hero.name %> {{retired}} to " +
        god.select(
          "a life of farming",
          "write <%= possessive(hero) %> memoirs",
          "live in peace",
          "pine for days of adventure",
        ) +
        ".",
    ];
    ...
    if (lod.dead.length > 0) {
      ...
      var sent =
        "Years {{passed}}, but <%= hero.name %> still {{mourns}} the stinging loss of " +
        god.list(lod.dead) +
        ".";
      t.push(sent);
    }
    ...
    t.push(
      "After that <%= pronoun(hero) %> lived long and happily, survived to a great age, and then died peacefully.",
    );
  ...
  story.latd = function (person, god) {
    ...
    if (person.acquaintances) {
      people = people.concat(person.acquaintances);
    }
  ...
  story.outro = function (god) {
    ...
    if (narr || (god.coinflip(0.2) && ld.living.length > 0)) {
      ...
      t.push(
        (god.coinflip() ? "This may sound fantastic, but " : "") +
          "in all the world there is nothing stranger than the truth, and it all happened exactly as I have told you, for I was there, as sure as my name is " +
          narr +
          ".",
      );
    } else {
