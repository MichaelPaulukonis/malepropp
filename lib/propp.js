...
import createCharacterHelpers from "./god/characters.js";
...
// fnq.4: tracks nested generate() calls (lib/templates.js recurses into a
// full new storyGen(...).generate(...) for story-within-a-story functions),
// so only the outermost generate() unseeds on exit - see the try/finally in
// generate() below.
var generateDepth = 0;
...
var storyGen = function (settings) {
  ...
  // Both arguments must come from this app's own internal templates/god()
  // data, never external input - new Function executes templateString/data
  // directly as code.
  var interpolate = function (templateString, data) {
    var keys = Object.keys(data);
    var body = templateString.replace(
      /<%=\s*([\s\S]+?)\s*%>/g,
      function (match, expr) {
        // ?? evaluates expr exactly once - a naive (expr) == null ? '' : (expr)
        // would evaluate expr twice, and several real templates call
        // non-deterministic helpers (coinflip(), pick()) inside tags, so a
        // double-evaluated tag can silently pick a different random branch
        // for the discarded null-check than for the actual output.
        // biome-ignore lint/style/useTemplate: string concatenation needed for Function constructor
        return "${(" + expr + ") ?? ''}";
      },
    );
    // biome-ignore lint/style/useTemplate: string concatenation needed for Function constructor
  ...
  var god = function (settings, wordbank, t) {
    ...
    var getPlace = function (id) {
      var p;
      if (cache.places && cache.places[id]) {
        p = cache.places[id];
      }
      return p;
    };

    var characterHelpers = createCharacterHelpers({
      world: world,
      bank: bank,
      cache: cache,
      settings: settings,
      ...
      uid: uid,
      place: place,
      pronounHelpers: pronounHelpers,
    });
    var createCharacter = characterHelpers.createCharacter;
    var createCharacters = characterHelpers.createCharacters;
    var createFamily = characterHelpers.createFamily;
    var createHero = characterHelpers.createHero;
    var createVillain = characterHelpers.createVillain;
    var createMagicalHelper = characterHelpers.createMagicalHelper;
    var createHome = characterHelpers.createHome;
    var createFalsehero = characterHelpers.createFalsehero;
    var createMagicalitem = characterHelpers.createMagicalitem;
    var createPunished = characterHelpers.createPunished;
    var getCharacter = characterHelpers.getCharacter;
    ...
    var getName = function (thing, property) {
      ...
      if (property && thing[property]) {
        return getCharacter(thing)[property];
      }
      // if thing starts with "id_" then it is a character reference
      // or another sort of reference. hrm.
      if (thing.indexOf("id_") == 0) {
        property = property || select("name", "nickname");
      }
      var elem =
        typeof thing === "string" && thing.indexOf("id_") == -1
          ? thing
          : getCharacter(thing)[property];
      return elem;
    };
    ...
    var converse = function (p1, p2) {
      ...
      var says =
        '{{<%= select("said", "remarked", "noted", "mused", "exclaimed", "ejected", "rumbled", "muttered") %>}}';
      var reply =
        '{{<%= select("replied", "responded", "retorted", "volleyed", "returned", "muttered") %>}}';
      ...
      if (p1 && p2) {
        ...
        c.push(
          '"Yes, I am," {{conceded}} {{P1N}}. "But it\'s been said that I\'m also ' +
            p1.description[1] +
            '!"',
        );
      } else if (p1 && !p2) {
        ...
        c.push(
          '"' +
            capitalize(pick(bank.interjections)) +
            select("!", ",") +
            '" {{SAY}} {{P1N}}' +
            (coinflip() ? " to nobody in particular." : ".") +
            (coinflip()
              ? ""
              : ' "' + capitalize(pick(bank.interjections)) + punct() + '"'),
        );
      }
      ...
      if (t === "bad") {
        if (coinflip()) {
          c.push(
            world.blankLine,
            '"Don\'t go bragging like that!" says {{P2N}}',
          );
        }
      }
      var tmpl = c
        ...
        .replace(/{{P2N}}/gm, p2n)
        .replace(/{{SAY}}/gm, says)
        .replace(/{{RPLY}}/gm, reply)
        .replace(/{{GG}}/gm, function () {
          return pick(bank.greetings[t]);
        });
    ...
    var init = (function () {
      ...
      try {
        ...
        if (!settings.herogender || settings.herogender === "random") {
          settings.herogender = world.util.randomProperty(world.gender);
        }
        if (!settings.villaingender || settings.villaingender === "random") {
          settings.villaingender = world.util.randomProperty(world.gender);
        }
        if (!settings.peoplegender || settings.peoplegender === "random") {
          settings.peoplegender = world.util.randomProperty(world.gender);
        }

        if (settings.narrator) {
          cache.narrator = settings.narrator;
        }
        ...
        cache.hero =
          settings.hero || createHero(settings.herogender, world.aspect.good);
        cache.villain =
          settings.villain ||
          createVillain(
            settings.villaingender,
            world.aspect.bad,
            createMagicalitem(),
            2,
          );
        ...
        cache.advisor =
          settings.advisor || createCharacter(null, world.aspect.good);
        cache.advisor.introduced = false; // ugh.
        cache.magicalitem = settings.magicalitem || createMagicalitem();
        cache.magicalhelper =
          settings.magicalhelper ||
          createMagicalHelper(null, world.aspect.good);
  ...
  var sentence = function (func, helper, params, verbtense) {
    ...
    if ((func && func.active) || (func && isFunc)) {
      ...
      } else if (func.templates && func.templates.length) {
      ...
      f = f.replace(/{{\*\*}}/gm, ""); // so remove it
      if (f.indexOf("{{**}}") >= 0) {
        f = f.replace(/{{\*\*}}/gm, ""); // remove it
        f = f.replace(/{{|}}/gm, ""); // remove old-style verb-tags
        ...
        for (var i = 0; i < sentences.length; i++) {
          ...
          if (verbtense == "past") {
            tensed.push(
              capitalize(nlp.pos(sentences[i]).sentences[0].to_past().text()),
            );
          } else {
            tensed.push(
              capitalize(
                nlp.pos(sentences[i]).sentences[0].to_present().text(),
              ),
            );
          }
        }
      ...
      while ((tag = re.exec(f)) !== null) {
        ...
        // nlp is window.nlp_compromise in the browser, the dynamically-
        // imported npm package in Node (see the top of this file) -
        // behavior may still differ slightly between the two pinned
        // versions.
        var tense;
        if (verbtense == "past") {
      ...
      // exceptions
      f = f
        .replace(/wered/gm, "were")
        .replace(/weres/gm, "are")
        .replace(/strided/gm, "strode")
        .replace(/wased/gim, "WAS");
  ...
  var generate = function (settings, theme) {
    // fnq.4: rngState is module-scope (see storyGen.seed above), so a seed
    // set here - or left behind by a caller that forgot unseed() - would
    // otherwise leak into every later generate() call in the same warm
    // process (e.g. a reused Lambda container). generate() is the engine's
    // settings-in/tale-out boundary, so it always reverts to real
    // Math.random() when it's done, seeded or not, success or throw.
    // generateDepth guards against unseeding too early: lib/templates.js
    // nests a full `new storyGen(...).generate(...)` call inside an outer
    // generate() (a story-within-a-story), so only the outermost call is
    // allowed to unseed - an inner call finishing must not cut off the
    // outer call's still-in-progress seeded draws.
    generateDepth++;
    try {
