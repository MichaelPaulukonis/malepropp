...
var storyGen = function (settings) {
  ...
  var generate = function (settings, theme) {
    ...
    // lib/templates.js reads sg.universe after calling sg.generate(...) (a method
    // call, so `this` is the storyGen instance there) - keep this assigned for that
    // path. Guarded because `this` is undefined when generate() is called detached
    // (destructured) under strict-mode/ESM default-`this` rules - see the
    // "this-binding independence" tests in tests/malepropp.tests.js.
    if (this) {
