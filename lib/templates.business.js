var businessTemplates = function (story, world, storyGen) {
  ...
  story.title = function (god) {
    ...
    var templates = [
      ...
      (god.coinflip() ? "The Doings of " : "") +
        "{{SN}} of <%= hero.home.nation %>",
      (god.coinflip() ? "Employee " : "") +
  ...
  story.createLack = function (god) {
    ...
    var lacks = [
      "{{needs}} an mentor, an intern, or just somebody to talk to",
      "{{needs}} a mentor or business case",
      [
        "{{needs}} a patent or two",
        "Possibly three. No more than that. Unless they {{were}} collectible",
      ],
      [
        "{{needs}} articles of incorporate of a declaration of bankruptcy",
        "Either would do",
      ],
  ...
  story["func7"].exec = function (god) {
    ...
    var templates = ["{{HN}} unwittingly helps {{VN}}."];
  ...
  story["func8"].exec = function (god, subFunc, skipIntros) {
    ...
    switch (subFunc) {
      case "kidnapping of person":
        templates.push(
          "{{VN}} kidnaps <%= select(hero.family, hero.acquaintances).name %>.",
        );
        break;

      case "seizure of magical agent or helper":
        templates.push(
          '{{VN}} <%= select("forcibly seizes", "kidnaps", "makes off with") %> <%= magicalhelper.name %>.',
        );
        break;

      case "forcible seizure of magical helper":
        templates.push(
          "The report is destroyed by {{VN}}. All begin to feel the discomfort of chaos.",
        );
        break;
      ...
      case "theft of daylight":
        templates.push(
          "Suddenly, it becomes as night. {{VN}} has shut off the power!",
        );
        break;
      ...
      case "bride is forgotten":
        ...
        templates.push(
          "{{HN}}'s intern is forgotten after {{VN}} casts a spell.",
        );
        break;

      case "demand for delivery or enticement, abduction":
        templates.push(
          "{{VN}} makes a demand for delivery or enticement, abduction.",
        );
        break;
      ...
      case "casting into body of water":
        templates.push(
          '{{VN}} throws {{HN}} into <%= select("the toilet", "a drinking fountain", "the decorative fountain in the lobby") %>.',
        );
        break;

      case "casting of a spell, transformation":
        templates.push(
          "It is hard to describe the charged atmosphere once {{VN}} has their policies enacted.",
        );
        break;
      ...
      case "threat of forced matrimony":
        ...
        templates.push(
          "{{VN}} threatens to fire <%= pick(hero.family).name %>.",
        );
        break;

      case "threat of forced matrimony between relatives":
        templates.push(
          '{{VN}} <%= select("insinuates", "suggests", "muses") %> that <%= list(hero.family) %> could be forced to transfer to his/her division.',
        );
        break;
      ...
      case "threat of cannibalism among relatives":
        templates.push(
          "Thanks to the ravages {{VN}}'s disruptions have left on the firm, there is the threat of layoffs among {{HN}}'s work-group. <%= list(hero.family) %> eye each other nervously.",
        );
        break;
      ...
      case "tormenting at night (visitation, vampirism)":
        ...
        templates.push(
          "{{HN}} is tormented during meetings by <%= pick(villain.family).name %>.",
        );
        break;
  ...
  story["func9"].exec = function (god) {
    ...
    var templates = ["{{HN}} discovers that " + name + " " + lack.lack];
  ...
  story["func10"].exec = function (god) {
    ...
    var templates = ["{{HN}} chooses positive action."];
  ...
  story["func16"].exec = function (god) {
    ...
    var templates = ["{{HN}} and {{VN}} do battle."];
  ...
  story["func19"].exec = function (god, subFunc) {
    ...
    var templates = ["Initial misfortune or lack is resolved."];
  ...
  story["func23"].templates.push(
    "{{HN}} arrived in <%= hero.home.vicinity %> but {{was}} unrecognized.",
  );
  ...
  story["func30"].exec = function (god, subFunc) {
    ...
    var templates = ["{{VN}} is <%= punished() %> by {{HN}}."];
    ...
    if (narr || (god.coinflip(0.2) && ld.living.length > 0)) {
      ...
      t.push(
        (god.coinflip() ? "This may sound fantastic, but " : "") +
          "in all the world there is nothing stranger than the truth, and it all happened exactly as I have told you, for I was there as an auditor, as sure as my name is " +
          narr +
          ".",
      );
    } else {
