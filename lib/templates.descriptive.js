...
var descriptiveTemplates = function(propp) {
    ...
    propp["func0"].templates.push("Someone close to the <%= hero() %> goes missing.");
    ...
    propp["func7"].templates.push("Villainy and lack: The need is identified (Villainy).");

    //  Villainy and lack: The need is identified (Lack)
    propp["func8"].templates.push("Villainy and lack: The need is identified (Lack)");
    ...
    propp["func10"].templates.push("Counteraction: <%= hero() %> chooses positive action.");
    ...
    propp["func12"].templates.push("Testing: <%= hero() %> is challenged to prove <%= hero() %>ic qualities.");
    ...
    propp["func14"].templates.push(" Acquisition: <%= hero() %> gains magical item.");

    // Guidance: hero reaches destination
    propp["func15"].templates.push("Guidance: <%= hero() %> reaches destination.");

    // Struggle: hero and villain do battle
    propp["func16"].templates.push("Struggle: <%= hero() %> and villain do battle.");
    ...
    propp["func19"].templates.push("Resolution: Initial misfortune or lack is resolved.");
    ...
    propp["func23"].templates.push("Arrival: <%= hero() %> arrives unrecognized.");

    // Claim: False hero makes unfounded claims
    propp["func24"].templates.push("Claim: False <%= hero() %> makes unfounded claims.");

    // Task: Difficult task proposed to the <%= hero() %>
    propp["func25"].templates.push("Task: Difficult task proposed to the <%= hero() %>.");
    ...
    propp["func29"].templates.push("Transfiguration: <%= hero() %> is given a new appearance.");

    // Punishment: Villain is punished
    propp["func30"].templates.push("Punishment: Villain is punished.");

    // Wedding: hero marries and ascends the throne
    propp["func31"].templates.push("Wedding: <%= hero() %> marries and ascends the throne.");
